...
@section('content')
    @include('users.partials.header', [
       ...
       'description' => __('On this page you can edit a config item of the system'),
       'class' => 'col-lg-7'
   ])
                        ...
                        <form action="{{route("settings.update", $item->id)}}" method="POST" autocomplete="off">
                            ...
                            <div class="form-group{{ $errors->has('key') ? ' has-danger' : '' }}">
                                ...
                                <div class="autocomplete">
                                    <input type="text" name="key" id="input-key" class="form-control form-control-alternative{{ $errors->has('key') ? ' is-invalid' : '' }}" placeholder="{{ __('Fill in a key') }}" value="{{ old("key", $item->key) }}">
                                </div>
                                @if ($errors->has('key'))
                            ...
                            <div class="form-group{{ $errors->has('value') ? ' has-danger' : '' }}">
                                ...
                                <div class="autocomplete">
                                    <input type="text" name="value" id="input-value" class="form-control form-control-alternative{{ $errors->has('value') ? ' is-invalid' : '' }}" placeholder="{{ __('Fill in a value') }}" value="{{ old("value", $item->value) }}">
                                </div>
                                @if ($errors->has('value'))
                                    <span class="invalid-feedback" role="alert">
